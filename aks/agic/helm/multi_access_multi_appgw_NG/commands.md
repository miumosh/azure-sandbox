...
**確認観点:**
- `agic-private-ingress-azure-*` が `Running 1/1`
- `agic-public-ingress-azure-*` が `Running 1/1`
...
# 全 AGIC Pod
kubectl logs -n kube-system -l app=ingress-azure --tail=100
```
...
| AGIC 2 インスタンス | `kubernetes.ingressClass` で Private / Public 各 Ingress の処理先を分離。release name が異なると SA 名も変わるため Federated Credential の subject を一致させること。 |
